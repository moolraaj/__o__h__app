...
const DiseaseList = () => {
  ...
  const { data: diseasesData, isLoading, refetch } = useGetFaqsQuery({ page: page, limit: PAGE_PER_ITEMS, lang: "en" });
  const [deleteDisease] = useDeletegetFaqMutation();
  const [showModal, setShowModal] = useState(false);
  const [selectedDiseaseId, setSelectedDiseaseId] = useState<string | null>(null);
  const handleDelete = async (id: string) => {
    try {
  ...
  useEffect(() => {
    setRightContent(
      <Link href="/super-admin/faqs/add-faq" className="add-slider-btn">
  ...
  return (
    <div className="disease-main-container">
      {isLoading ? (
        ...
        <div className="disease-list-container">
          {diseasesData?.result?.map((disease: ReplacedFaqType) => (
            <div key={disease._id} className="disease-card">
               
              <div className="disease-info">
                <p><b>Main Title:</b> {disease?.faqs_title?.en}</p>
              </div>
              <div className="disease-actions">
                <Link href={`/super-admin/faqs/update-faq/${disease._id}`} className="edit-button">
                  <span className="iconsss">
                    <FaEdit /> Edit
                  </span>
                </Link>
