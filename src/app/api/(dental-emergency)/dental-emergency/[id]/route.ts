...
export async function PUT(req: NextRequest, { params }: RouteContext) {
    try {
      ...
      const updateBilingualField = (fieldName: keyof DentalEmergencyTypes) => {
        ...
        if (fieldValue) {
          try {
            const parsed = JSON.parse(fieldValue.toString());
       
            updateData[fieldName] = { en: parsed.en, kn: parsed.kn };
          } catch (error) {
      ...
      const updateImageField = async (
        fileFieldKey: keyof DentalEmergencyTypes,
        docFieldKey: keyof DentalEmergencyTypes
      ) => {
        const fileField = formData.get(fileFieldKey);
        if (fileField && fileField instanceof File && fileField.size > 0) {
          const url = await uploadPhotoToCloudinary(fileField);
          updateData[docFieldKey] = url;
        }
      };
      ...
      console.error('Error updating DentalEmergency:', err);
