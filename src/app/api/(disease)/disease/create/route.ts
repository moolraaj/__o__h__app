...
import { Cause, CauseRepeat, PreventionTip, PreventionTipRepeat, Symptom, SymptomRepeat, TreatmentOption, TreatmentOptionRepeat, WhatIsDiseaseRepeat } from '@/utils/Types';
  ...
  try {
    ...
    what_is_disease_repeat = await Promise.all(
      what_is_disease_repeat.map(async (item, index) => {
        ...
        let imageUrls: string[] = Array.isArray(item.what_is_disease_repeat_images)
          ? item.what_is_disease_repeat_images
          : [];
        ...
        if (imageUrls.length === 0) {
          ...
          if (files.length) {
            imageUrls = await Promise.all(
              files.map(async (file) => await uploadPhotoToCloudinary(file))
            );
          }
        }
    ...
    console.error(err);
    return NextResponse.json({ success: false, message: 'Failed to create disease' }, { status: 500 });
  }
}
